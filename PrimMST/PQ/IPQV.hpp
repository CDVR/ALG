...
template <typename Idx, typename Prio, typename Value>
class IPQV {

public: //Renaming stuff
	using Duo = std::pair<Idx, Prio>;
	using Trio = std::pair<std::pair<Idx, Prio>, Value>;
...
private: //Helper methods
	void m_swap(const size_t& posA, const size_t& posB) {
		...
		std::swap(m_vals[posA], m_vals[posB]);
	}
...
public: //Methods
	Trio pollTop() {
		...
		if (!m_size) return Trio();
		m_size--; //Save the old value
		Trio saved = {m_heap[0], m_vals[0]};
		...
		m_heap.pop_back(); m_vals.pop_back();
	...
	void add(const Idx& idx, const Prio& prio, const Value& val) {
		//Getting the iterator to the presumed index
		auto it = m_table.find(idx);
		...
		if (it == m_table.end()) {
			...
			m_heap.push_back({idx, prio}); m_vals.push_back(val);
		...
		m_heap[it->second].second = prio; m_vals[it->second] = val;
	...
	bool addConditional(const Idx& idx, const Prio& prio, const Value& val) {
		//Getting the iterator to the presumed index
		auto it = m_table.find(idx);
		//Check the iterator if the element exists
		if (it == m_table.end()) {
	...
	Trio peekTop() {if (!m_size) return Trio(); return {m_heap[0], m_vals[0]};}	
	size_t size() {return m_size;}

public: //Constructor / destructor
	IPQV(const size_t& bcnt, const bool& minheap = true) : 
		m_size(0), m_minHeap(minheap), m_heap(Heap()), m_table(bcnt), m_vals(Vals()) {}

	IPQV(const std::initializer_list<Trio>& list) : IPQV(list.size()*3) {
		for (const auto& pair : list) addConditional(pair.first.first, pair.first.second, pair.second);
	}
